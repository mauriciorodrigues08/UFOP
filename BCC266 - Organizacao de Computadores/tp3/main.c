...
void gerar_TI(int qnt_inst, int tamRam);

void printResultado(Maquina *maquina);
void calculaPorcentagem(Maquina *maquina, double *pct1, double *pct2, double *pct3, double *pctRam, double *pctHD);
...
int main(){
  ...
  int num_instrucoes_TI = 100;
  gerar_TI(num_instrucoes_TI, TAM_RAM);

  //cria a máquina
  Maquina *maquina = criaMaquina();

  Instrucao *instrucoes = criaInstrucoes(num_instrucoes);
  Instrucao* tratador_interrupcao = criaInstrucoes(num_instrucoes_TI);

  //lê as instruções do arquivo gerado
  recebe_instrucoes(instrucoes, num_instrucoes, "instrucoes.txt");
  recebe_instrucoes(tratador_interrupcao, num_instrucoes_TI, "tratador.txt");

  //roda a máquina
  int prob_interrupcao;
  printf("\nInsira a probabilidade de interrupção a cada instrução (1 - 100): ");
  scanf("%d", &prob_interrupcao);

  iniciar(maquina, instrucoes, tratador_interrupcao, prob_interrupcao);

  //printa resultado
  printResultado(maquina);

  //libera vetor de instruções e máquina
  instrucoes = destroiInstrucoes(instrucoes);
  tratador_interrupcao = destroiInstrucoes(tratador_interrupcao);
...
void gerar_TI(int qnt_inst, int tamRam) {
    FILE *arquivo = fopen("tratador.txt", "w");
    if (!arquivo) {
        printf("Arquivo não encontrado!\n");
        return;
    }

    for (int i = 0; i < qnt_inst - 1; i++) {
        fprintf(arquivo, "%d %d %d %d %d %d %d\n",
            rand() % 3, rand() % tamRam, rand() % NUM_PALAVRAS,
            rand() % tamRam, rand() % NUM_PALAVRAS, rand() % tamRam, rand() % NUM_PALAVRAS
        );
    }
...
void printResultado(Maquina *maquina){
  ...
  double porcentagem1, porcentagem2, porcentagem3, porcentagemRam, porcentagemHD;

  calculaPorcentagem(maquina, &porcentagem1, &porcentagem2, &porcentagem3, &porcentagemRam, &porcentagemHD);

  printf("\n===================== RESULTADOS =====================\n");
  printf("Ciclos estimados da maquina: %ld\n", maquina->ciclos);
  printf("Custo total: %ld\n", maquina->custo);
  printf("======================================================\n");
  printf("Cache 1 -> Hit: %8ld | Miss: %8ld | %.3lf%%\n", hit_miss_cache1[0], hit_miss_cache1[1], porcentagem1);
  printf("Cache 2 -> Hit: %8ld | Miss: %8ld | %.3lf%%\n", hit_miss_cache2[0], hit_miss_cache2[1], porcentagem2);
  printf("Cache 3 -> Hit: %8ld | Miss: %8ld | %.3lf%%\n", hit_miss_cache3[0], hit_miss_cache3[1], porcentagem3);
  printf("RAM     -> Hit: %8ld | Miss: %8ld | %.3lf%%\n", maquina->ram->ramHit, maquina->ram->ramMiss, porcentagemRam);
  printf("HD      -> Hit: %8ld |        -       | %.3lf%%\n", maquina->hd->HDhit, porcentagemHD);
  printf("======================================================\n");
}
...
void calculaPorcentagem(Maquina *maquina, double *pct1, double *pct2, double *pct3, double *pctRam, double *pctHD){
  ...
  *pctRam = (double) (100 * result);
  ...
  result = (double) maquina->hd->HDhit/soma;
  *pctHD = (double) (100 * result);
}
